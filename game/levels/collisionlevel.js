...
export class CollisionLevel extends BaseLevel {
    ...
    update(delta) {
        switch (this.gameState) {
            ...
            case GameState.PLAYING:
                ...
                let xxx = 0, yyy = 0; //, lineLenght:number=100;
                let i = 1;
                for (; i < this.lineLenght &&
                    !CollisionUtil.getInstance().spritePointCollision(xxx, yyy, this.blueSprite); i++) {
    ...
    drawLaser(ctx) {
        ...
        ctx.lineTo(this.arrow.x + MathUtil.lengthDirX(i, this.arrow.angle), this.arrow.y + MathUtil.lengthDirY(i, this.arrow.angle));
        ctx.stroke();
    } //
    loadImages() {
