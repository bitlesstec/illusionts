...
export class TileUtil {
     ...
     * @param tileHeight
     */
    static parse(tileMap, cols, rows, tileWidth, tileHeight) {
        let tileList = [];
        let tileIndex = 0;
     ...
     * @param tiles
     */
    static renderTiles(ctx, img, tiles) {
        // using lambda
        // tiles.forEach( (tile) => 
        // {
        //     ctx.drawImage
        ...
        for (let x = 0; x < tiles.length; x++) {
            ctx.drawImage(img, tiles[x].imageIndex * tiles[x].w, 0, tiles[x].w, tiles[x].h, Math.ceil(tiles[x].x), Math.ceil(tiles[x].y), tiles[x].w, tiles[x].h);
        }
    } //
} //
